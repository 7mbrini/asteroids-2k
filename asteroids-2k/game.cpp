...
BOOL CALLBACK BestScoresDlgProc(HWND hDlg, UINT nMessage, WPARAM wParam, LPARAM lParam)
{
	...
	{
		case WM_INITDIALOG:
			//SetDlgItemText(hDlg, IDC_EDIT_NAME, L"Text");
		return TRUE;
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case(IDOK):
					...
					GetDlgItemText(hDlg, IDC_EDIT_NAME, g_pGame->pBestScoresName, 16);	// considera solo i primi 16 caratteri
...
void GameOverHandler(TGame* pGame)
{
	...
	int nItems = min(BESTSCORES, pGame->BestScores.size());
...
bool IsBestScore(TGame* pGame)
{
	...
	int nCount = min(BESTSCORES, pGame->BestScores.size());
